...
class ProductsController extends Controller
{
    ...
    public function getadd($id)
    {
        ...
        $pro    = Products::all(); 
        if ($p_id >=19) {
                return view('back-end.products.add-pc',['data'=>$pro,'cat'=>$cat,'loai'=>$p_name->name]);
            }
        else {
            return view('back-end.products.add-mobile',['data'=>$pro,'cat'=>$cat,'loai'=>$p_name->name]);
        }
    }
    ...
    public function getEdit($loai,$id)
    {
        ...
        if ($p_id == 1) {
            ...
            $pro = Products::where('id',$id)->first();
            return view('back-end.products.edit-pc',['pro'=>$pro,'cat'=>$cat,'loai'=>'Laptop']);       
        } elseif ($p_id ==19) {
            $cat = Category::where('parent_id', 19)->get();
            $pro = Products::where('id',$id)->first();
            return view('back-end.products.edit-pc',['pro'=>$pro,'cat'=>$cat,'loai'=>$p_id]);     
        }
    }
     ...
     * @param  [type]              $loai [description]
     * @param  [type]              $id   [description]
     * @param  EditProductsRequest $rq   [description]
     * @return [type]                    [description]
     */
